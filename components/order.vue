                      ...
                      <template v-for="(data, idx) in order_data.win_stdwin_subtype.option_values">
                        <div class="picOption" v-if="show_win_stdwin_sbutype(data.id)">
                          <VeeField type="checkbox" :id="'win_stdwin_subtype_' + data.id"
                            ...
                            @click="checkbox_value(data.id,'win_stdwin_subtype')" 
                            :value="data.id" 
                    ...
                    <div class="d-flex align-items-center gap-3 flex-wrap p-2"
                      ...
                      <template v-for="(data,idx) in order_data.outer_frame_cut_position.option_values" :key="data.id">
                        <label :for="'outer_frame_cut_position' + data.id">
                          <VeeField 
                            type="radio" 
                            :name="`${order_data.outer_frame_cut_position.code}.option_values[${idx}].option_value_id`" 
                            :value="data.id"
                        ...
                        <!-- <VeeErrorMessage class="error__label" :name="`${order_data.outer_frame_cut_position.code}.option_values[${idx}].option_value_id`" /> -->
                    </div>
                  </div>
                </div>
                <!-- 鑽孔 -->
                ...
                <div class="row mb-3" v-if="values[order_data.outer_frame_gasket_location.code]?.option_value_id == 2219">
                  ...
                  <div class="col-sm-8">
                    <VeeField type="text" class="form-control"
                      ...
                      v-model="getProductOptionValue(order_data.wing_spacer_thickness.code).value"
                ...
                <div class="row mb-3" v-if="values[order_data.outer_frame_gasket_location.code]?.option_value_id == 2219">
                  ...
                  <div class="col-sm-8">
                    <VeeField type="text" class="form-control"
                      ...
                      v-model="getProductOptionValue(order_data.bottom_spacer_thickness.code).value"
          ...
          <div class="block block3 mainForm">
            ...
            <!-- 拉桿 -->
              ...
              <div class="col-md-6 flex-auto">
                  ...
                  <div class="col-sm-8">
                    <div class="d-flex align-items-center gap-3 flex-wrap p-2"
                      ...
                      <label :for="'handle_pre_locked' + data.id" v-for="data in order_data.handle_pre_locked.option_values"
                        :key="data.id">
                        <VeeField type="radio" :name="`${order_data.handle_pre_locked.code}.option_value_id`" :value="data.id"
                        v-model="order_fill_out_id.handle_pre_locked"
                          class="form-check-input" :id="'handle_pre_locked' + data.id"
                          @change="order_fill_out_id.handle_pre_locked = data.id" label="handle_pre_locked"
                          rules="required" />
                        ...
                        v-model="set_name(order_data.handle_pre_locked,order_fill_out_id.handle_pre_locked).value"
                         />
                        <span class="ms-2">{{ data.name }}</span>
                ...
                <div class="row mb-3" v-if="!is_none_handle && !is_aluminum && values.handle_pre_locked?.option_value_id == '2372'">
                  ...
                  <div class="col-sm-8">
                    <div class="d-flex align-items-center gap-3 flex-wrap p-2"
                      ...
                      <label :for="'lock_pre_locked' + data.id" v-for="data in order_data.lock_pre_locked.option_values"
                        ...
                        v-model="order_fill_out_id.lock_pre_locked"
                          :disabled="is_lock_type1"
                          class="form-check-input" :id="'lock_pre_locked' + data.id"
                          @change="order_fill_out_id.lock_pre_locked = data.id" label="lock_pre_locked"
                          rules="required" />
                        ...
                        v-model="set_name(order_data.lock_pre_locked,order_fill_out_id.lock_pre_locked).value"
                         />
                        <span class="ms-2">{{ data.name }}</span>
          ...
          <div class="block block3 mainForm">
            ...
            <div class="row flex-auto col-12">
              ...
              <div class="col-md-12" v-if="show_add_lever &&  !is_none_win_lever">
                <div class="d-none" v-if="order_fill_out_id.lever_segmentation.length > 0">
                ...
                <div class="row mb-3" v-for="(data,idx) in order_fill_out_id.lever_segmentation" :key="`lever_segmentation${idx +1}`">
                  ...
                  <div class="col-sm-4 position-relative">
                    <VeeField type="number" :name="`${order_data.lever_segmentation_note.code}.value[${idx}].lever_segmentation_value`"
                      @keydown="clearInput"
                      v-model="data.lever_segmentation_value"
                      :id="`${order_data.lever_segmentation_note.name}${idx + 1}`" class="form-control"
                      :label="`lever_segmentation${idx + 1}`"/>
                      <i class="fas fa-times close_btn" @click="del_lever_segmentation(idx)"></i>
                  </div>
                </div>
              </div>
              <!-- input_hiiden -->
              ...
              <div class="col-12 text-center mt-4 d-flex align-items-center justify-content-center gap-4">
                <button type="button" class="add_text_btn" @click="add_lever_segmentation" :disabled="order_fill_out.partitio.length > 0">
        ...
        <div class="container container-original">
          <div class="block block3 mainForm">
            ...
            <div class="row flex-auto">
                    ...
                    <div class="col-sm-8">
                      <div class="d-flex align-items-center gap-3 flex-wrap p-2"
                        ...
                        <label :for="'top_Partition_cut' + data.id"
                          ...
                          <VeeField type="radio" :name="`${order_data.top_board_cut.code}.option_value_id`"
                            :value="data.id" class="form-check-input"
                            v-model="order_fill_out_id.upper_track_cut"
                            :id="'top_Partition_cut' + data.id" @change="order_fill_out_id.upper_track_cut = data.id"
                            label="top_Partition" rules="required" />
                          ...
                          v-model="set_name(order_data.top_board_cut,order_fill_out_id.upper_track_cut).value"
                          />
                          <span class="ms-2">{{ data.name }}</span>
                        </label>
                      </div>
                    ...
                    <div class="col-sm-8">
                      <div class="d-flex align-items-center gap-3 flex-wrap p-2"
                        ...
                        <label :for="'top_Partition_color' + data.id"
                          ...
                          <VeeField type="radio" :name="`${order_data.upper_track_color.code}.option_value_id`"
                            :value="data.id" class="form-check-input"
                            v-model="order_fill_out_id.upper_track_color"
                            :id="'top_Partition_color' + data.id" @change="order_fill_out_id.upper_track_color = data.id"
                            label="top_Partition" rules="required" />
              ...
              <hr>
              <!-- BOTTOM -->
              <div class="col-12 row flex-auto"  v-if="!!doorType_track">
                <div class="col-6">
                  <!-- 底板 -->
                  <div class="row mb-3">
                    <label class="label-set col-form-label col-sm-4 " for="">
                    ...
                    <div class="col-sm-8">
                      <div class="d-flex align-items-center gap-3 flex-wrap p-2"
                        ...
                        <label :for="'top_Partition_cut' + data.id"
                          ...
                          <VeeField type="radio" :name="`${order_data.bottom_board_cut.code}.option_value_id`"
                            :value="data.id" class="form-check-input"
                            v-model="order_fill_out_id.lower_track_cut"
                            :id="'top_Partition_cut' + data.id" @change="order_fill_out_id.lower_track_cut = data.id"
                            label="top_Partition" rules="required" />
                          ...
                          v-model="set_name(order_data.bottom_board_cut,order_fill_out_id.lower_track_cut).value"
                          />
                          <span class="ms-2">{{ data.name }}</span>
                        </label>
                      </div>
                  ...
                  <div class="row mb-3">
                    ...
                    <div class="col-sm-8">
                      <div class="d-flex align-items-center gap-3 flex-wrap p-2"
                        ...
                        <label :for="'bottom_track' + data.option_value.id"
                          v-for="data in doorType_track.option_group_suboption_values" :key="data.option_value.id">
                          <VeeField type="radio" @change="order_fill_out_id.bottom_track = data.option_value.id"
                            :name="`${doorType_track.option_code}.option_value_id`"
                            v-model="order_fill_out_id.bottom_track" :value="data.option_value.id"
                    ...
                    <div class="col-sm-8">
                      <div class="d-flex align-items-center gap-3 flex-wrap p-2"
                        ...
                        <label :for="'bottom_track_color' + data.id"
                          ...
                          <VeeField type="radio" @change="order_fill_out_id.lower_track_color = data.id"
                            :name="`${order_data.lower_track_color.code}.option_value_id`"
                            v-model="order_fill_out_id.lower_track_color" :value="data.id"
              ...
              <!-- other -->
              <div class="col-12 row flex-auto">
                <div class="col-md-6 col-12 row mb-3 flex-auto" v-if="!is_folding_door">
                    <label class="label-set col-form-label col-sm-4 " for="">
                      <span class="text-danger">*</span>
      ...
      <section id="T-POST" v-if="!!order_fill_out_id.designType && !is_track">
        <div class="container container-original">
          <div class="block block3 mainForm">
            ...
            <div class="row flex-auto">
              <!-- T型式 -->
              <div class="col-md-12" :key="win_style_key" v-if="t_post_data">
                <div class="row mb-3">
                  <label class="label-set col-form-label col-sm-2" for="">
                    <span class="text-danger">*</span>
                    {{t_post_data.name}}
          ...
          <div class="block block3 mainForm">
            ...
            <div class="row flex-auto">
              <div class="col-md-6 mb-3">
                ...
                <!-- 轉角窗 -->
                <div class="d-none" v-if="order_fill_out_id.corner_angle.length > 0 && show_corner">
                ...
                <div class="row mb-3" v-for="(data,idx) in order_fill_out_id.corner_angle" :key="`corner_angle${idx +1}`" v-if="show_corner">
                  <span class="col-sm-2" v-if="idx == 0">轉角窗</span>
                  ...
                  <div class="col-sm-6 position-relative">
                    <VeeField type="number" :name="`${order_data.corner_angle.code}.value[${idx}].corner_angle_value`"
                      ...
                      <i class="fas fa-times close_btn" @click="del_corner_angle(idx)"></i>
                  </div>
                </div>
...
const order_fill_out_id = ref({
  ...
  pilar_type:"",
  pelmet_id:"",
  corner_angle:[{corner_angle_value:""}],
  indooroutdoor:"",
  std_or_fixed:"",
  sldwood_material:"",
...
const is_lock_type1 = computed(()=>{
  const is_type1 =  order_fill_out_id.value.lock_type == 2112
  if(is_type1){
    order_fill_out_id.value.lock_pre_locked = 2374
  }
  return is_type1
})
// 無下軌道
...
const t_post_data = computed(() => {
  const select_outer_frame = doorType_outerFrame.value.option_values.find(
    item => item.id === order_fill_out_id.value.outerFrame_type
  );
...
const checkbox_value = (id, code) => {
  let data = new Set(order_fill_out_id.value[code] || [])  // 使用 Set 來處理數據

  // id 在數組中則移除，否則添加
  data.has(id) ? data.delete(id) : data.add(id)
  if (code === 'outer_frame_cut_position') { // 切窗台
    if(id === 2222){ //選無刪掉其他選項
      data.clear()
      data.add(id)
    }else{
      data.delete(2222) //選其他則刪除無
    }
  }

    // 更新數據回去，轉換 Set 為數組
    order_fill_out_id.value[code] = [...data]
}
...
const track_img = (top_track)=>{
  let selected_id 
  if(top_track){ 
    selected_id = order_fill_out_id.value.upper_track_color //上軌道顏色
    return order_data.value.upper_track_color.option_values.find(item => item.id == selected_id)?.thumb
  }else{
    selected_id = order_fill_out_id.value.bottom_track //下軌道種類
...
// 拉桿分段
const lever_segmentation_data = ref({
  lever_segmentation_value:""
})

// 增加中隔
const add_lever_segmentation = ()=>{
  order_fill_out_id.value.lever_segmentation.push({...lever_segmentation_data.value})
}
// 刪除中隔
const del_lever_segmentation = (idx) =>{
  order_fill_out_id.value.lever_segmentation.splice(idx,1)
}
...
const corner_angle_data = ref({
  corner_angle_value:""
})

// 增加中隔
const add_corner_angle = ()=>{
  order_fill_out_id.value.corner_angle.push({...corner_angle_data.value})
}
// 刪除中隔
const del_corner_angle = (idx) =>{
  order_fill_out_id.value.corner_angle.splice(idx,1)
}
...
watch(() => product_options.value, async (newVal, oldVal) => {
  ...
  // 門片
    ...
    console.log(newVal);
    console.log(`${order_fill_out_id.value.material_code}_win_frame`);
  ...
  // 窗型
    ...
    if(is_track.value){
      ...
      order_fill_out_id.value.upper_track_cut = newVal.top_board_cut.option_value_id
      order_fill_out_id.value.bottom_track = newVal.door_down_track.option_value_id
      order_fill_out_id.value.lower_track_cut = newVal.bottom_board_cut.option_value_id
...
const set_name = (data,id) => {
  return computed({
    ...
    get: () => {
      ...
      if(data.option_id == '1002' || data.option_id == '1003' || data.option_id == '1004' || data.option_id == '1071' || data.option_id == '1053'){
