                      ...
                      <template v-for="(data, idx) in order_data.win_stdwin_subtype.option_values">
                        <div class="picOption" v-if="show_win_stdwin_subtype(data.id)">
                          <VeeField type="checkbox" :id="'win_stdwin_subtype_' + data.id"
                            ...
                            v-model="data_id[order_data.win_stdwin_subtype.code]"
                            :value="data.id"
                    ...
                    <div class="d-flex flex-wrap align-items-start  p-2 pt-0"
                      ...
                      <div class="picOption" v-for="item in order_data.door_handle_type.option_values" :key="item.id">
                        <VeeField type="radio" :id="'lockHandle' + item.id"
                          ...
                          @change="get_handle_color(item.id),set_name(item,order_data.door_handle_type.code)"
                    ...
                    <div class="d-flex flex-wrap align-items-start  p-2 pt-0"
                      ...
                      <div class="picOption" v-for="item in door_lock_type_data.option_values" :key="item.id">
                        <VeeField 
                          type="radio" 
                          :id="item.id" 
                          :name="`${door_lock_type_data.code}.option_value_id`"
                          v-model="data_id[door_lock_type_data.code]" 
                          :value="item.id"
                          :key="door_lock_type_data.code + 'option_value_id'"
                          @change="get_lock_color(item.id),set_name(item,door_lock_type_data.code)"
                          label="lock_type"
                          rules="required" />
          ...
          <div class="block block3 mainForm">
            ...
            <div class="row flex-auto col-12">
              ...
              <div class="col-12 text-center mt-4 d-flex align-items-center justify-content-center gap-4">
                <button type="button" class="add_text_btn" @click="add_json(order_data.lever_segmentation_note.code,lever_segmentation_obj)" :disabled="Divider_rail_data.length > 0">
                  <i class="fa-solid fa-circle-plus fs-4 add_btn"></i>
                  <span>{{order_data.lever_segmentation_note.name}}</span>
                </button>
                <button type="button" class="add_text_btn" @click="add_json(order_data.dividers_json.code,Divider_rail_obj)" :disabled="lever_segmentation_note_data.length > 0">
                  <i class="fa-solid fa-circle-plus fs-4 add_btn"></i>
...
const { validate,validateField } = useForm();//驗證套件
...
const is_none_bottom_track = computed(()=>{
  return data_id.value[doorType_track.value.option_code] == 2193
})
...
const is_none_post = computed(()=>{
  ...
  return t_post== 2209 || t_post == 2348
})
...
const get_order_data = async()=>{
  ...
  data_id.value[order_data.value.hinge_color.code] = 2100         // 絞鍊
