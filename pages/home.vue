            ...
            <div class="row mb-5" v-if="!store.loading && order">
                ...
                <div class="col-lg-4 col-md-6 mt-5" v-for="(item,idx) in order" :key="'order_' + idx">
                    <nuxt-link :to="`order-list/item?id=${item.id}`" class="home-order">
                        ...
                        <div class="pt-2"><span class="fw-bold">{{store.language_txt.order.text_orders}}{{store.language_txt.order.column_code}}：</span>{{item.code }}</div>
                        <div>
                            <span class="fw-bold">{{store.language_txt.order.column_customer_name}}：</span>{{item.customer_name}}
                        </div>
                        <div>
                            <span class="fw-bold">{{store.language_txt.member.column_address1}}：</span>{{item.shipping_address1}}{{ item.shipping_address2 }}
                        </div>
                        <div>
                            <span class="fw-bold">{{store.language_txt.order.column_total_quantity}}：</span>{{item.quantity}}
                        </div>
                        <div>
                            <span class="fw-bold">{{store.language_txt.order.column_sqm}}：</span>{{item.sqm}}
                        </div>
                        <div v-if="store.is_dealer || store.userData.parent_id == 0">
                            <span class="fw-bold">{{store.language_txt.order.text_order_comment_identity_salesperson}}：</span>{{item.salesperson_name}}
                        </div>
...
// 連結區塊
const link = ref([
    { icon: 'fa-plus', title: store.language_txt.order?.text_add_order, router: 'create_order', roles: ['salesperson', 'dealer','sunline'] },
    { icon: 'fa-history', title: store.language_txt.order?.tab_data, router: 'order-list', roles: ['salesperson'] },
    { icon: 'fa-clipboard-list', title: store.language_txt.order?.text_manage_order, router: 'order-list', roles: ['dealer','sunline'] },
    { icon: 'fa-file-circle-check', title: store.language_txt.order?.text_confirmed_orders, router: 'order-list?status=DealerApproved', roles: ['salesperson'] },
    { icon: 'fa-user-pen', title: store.language_txt.default?.text_basic_information, router: 'user-info', roles: ['salesperson', 'dealer','sunline'] },
    { icon: 'fa-gear', title: store.language_txt.order?.text_option_setting, router: '/order-setting', roles: ['dealer','sunline'] },
])
